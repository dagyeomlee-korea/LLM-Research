...
- CloudGoat 변경 matrix 96개의 CKA와 Huihui 0.8B/2B 변경 tensor 100개의 SVD를 완료했다.
- 전체 실행은 1분 31초, 최대 RSS 약 342MiB, exit code 0이었다.
- CloudGoat module별 CKA median은 MLP-up 0.99279, MLP-gate 0.99738, MLP-down 0.99905이며 attention은 0.99910~0.99964 범위다.
- CloudGoat CKA distance와 symmetric L2 상관은 0.92799, CKA distance와 kurtosis delta 상관은 -0.00356이다.
- Huihui의 모든 100개 변경 tensor는 rank-1에서 90% energy를 넘었다.
- Top-1 energy median은 0.8B 99.764%, 2B 99.553%다.
- Base→Instruct와 Instruct→Huihui delta alignment median은 각각 -0.000385, -0.000263으로 거의 직교한다.
- 결과는 NAS `~/qwen35-advanced-fingerprint-results`에 저장했다.
